...
# from raft import RAFT  # 使用RAFT光流模型
...
class SDM(nn.Module):
    def __init__(self, channels, rgb_channels, scale):
        ...
        # [修改点 1]: 空间融合 (Spatial Process)
        # 原版: InvBlock(DenseBlock, ...) -> 极重
        # 新版: 简单的 1x1 卷积 + 3x3 卷积融合
        ...
        # 频域融合 (保持不变，这是你论文的创新点 FAM/SDB)
        # 如果依然 OOM，可以考虑把这里的 channels 减半
        self.fre_process = SDB(channels, rgb_channels)
        ...
        self.avgpool = nn.AdaptiveAvgPool2d(1)
        self.contrast = stdv_channels  # 确保此函数在 common.py 前面已定义
...
class GCM(nn.Module):
    ...
    def forward(self, depth, rgb):
        ...
        # [修改点]：智能判断输入尺寸  只改了这里
        # 如果 depth 尺寸比 rgb 小，说明是初始 LR，需要上采样
        # 如果 depth 尺寸和 rgb 一样，说明是迭代中间结果(HR)，直接使用
